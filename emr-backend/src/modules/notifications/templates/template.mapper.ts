...
async function fetchDynamicTemplates(): Promise<Map<string, string>> {
    if (cachedTemplatesByName) {
        if (verboseEmailLogs) {
            logger.info('Using cached SendGrid dynamic templates', {
                templateCount: cachedTemplatesByName.size,
            });
        }
    ...
    if (verboseEmailLogs) {
        logger.info('Fetched SendGrid dynamic templates', {
            templateCount: templates.size,
            templateNames: Array.from(templates.keys()).sort(),
        });
    }
    return templates;
}
...
export async function resolveTemplateId(templateKey: NotificationTemplateKey): Promise<string> {
    ...
    if (configuredTemplateId) {
        if (verboseEmailLogs) {
            logger.info('Resolved SendGrid template from environment', {
                templateKey,
    ...
    if (discoveredTemplateId) {
        if (verboseEmailLogs) {
            logger.info('Resolved SendGrid template by dynamic template name', {
                templateKey,
                templateName: config.templateName,
                templateId: discoveredTemplateId,
            });
        }
