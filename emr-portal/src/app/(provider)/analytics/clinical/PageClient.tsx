    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 text-slate-900 dark:text-white dark:text-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Active Patients', value: '47', trend: '+12%', sub: 'Total enrollment', icon: Users, color: 'text-brand', bg: 'bg-brand/10 dark:bg-brand/20' },
                    { label: 'Avg Weight Loss %', value: '8.2%', trend: '+0.5%', sub: 'Avg across cohort', icon: Activity, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
                    ...
                    <div key={i} className="bg-white dark:bg-slate-800 dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm relative overflow-hidden group">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-black ${stat.trend.startsWith('+') ? 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400'}`}>
            ...
            <div className="grid grid-cols-12 gap-8">
                <div className="col-span-12 lg:col-span-8 bg-white dark:bg-slate-800 dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex gap-2">
                            <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-slate-500">
                ...
                <div className="col-span-12 lg:col-span-4 bg-slate-900 dark:bg-slate-950 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                    ...
                    <div className="flex-1 overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50/30 dark:bg-slate-900/30">
                                <tr>
                                    <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Patient</th>
                                    <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Overdue</th>
                                    <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Risk Level</th>
                            ...
                            <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                                {OVERDUE_LABS.map((p, i) => (
                                    <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-all group">
                                        ...
                                        <td className="px-8 py-4">
                                            <span className="text-xs font-black text-rose-500 bg-rose-50 dark:bg-rose-950/30 px-2 py-1 rounded-lg">{p.overdue}</span>
                                        </td>
                                        <td className="px-8 py-4">
                                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${p.status === 'High Risk' ? 'bg-rose-100 dark:bg-rose-900/50 text-rose-700 dark:text-rose-300' :
                                                    p.status === 'Medium' ? 'bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300'
                                                }`}>
                                                {p.status}
                                            </span>
                                        </td>
                                        <td className="px-8 py-4 text-right">
                                            <button className="p-2 text-slate-300 dark:text-slate-600 hover:text-brand transition-colors"><MessageSquare className="w-4 h-4" /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                <div className="bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 space-y-4">
                        {[
                            ...
                            <div key={i} className="flex gap-4 p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-900 transition-all border border-transparent hover:border-slate-100 dark:hover:border-slate-700 cursor-pointer">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className={`text-[8px] font-black uppercase tracking-tighter px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded`}>{insight.type}</span>
                                        <span className="text-[8px] font-bold text-slate-300 dark:text-slate-600">Logged 4h ago</span>
                                    </div>
                                </div>
                            </div>
                        ))}
