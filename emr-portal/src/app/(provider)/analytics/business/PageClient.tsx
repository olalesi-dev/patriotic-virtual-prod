    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 text-slate-900 dark:text-white dark:text-slate-100">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white dark:text-slate-100 tracking-tight">BUSINESS ANALYTICS</h1>
                    <p className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Growth, revenue, and operational efficiency</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex bg-white dark:bg-slate-800 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl p-1 shadow-sm">
                        ...
                        <button className="px-4 py-2 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300">Unit Economics</button>
                    </div>
                    ...
                    <button className="flex items-center gap-2 bg-slate-900 dark:bg-slate-700 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-slate-200 dark:shadow-none hover:bg-slate-800 dark:hover:bg-slate-600 transition-all active:scale-95">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'MRR', value: '$18,800', trend: '+14%', sub: 'Monthly Recurring', icon: DollarSign, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
                    { label: 'Avg Rev / Patient', value: '$400', trend: '+2.5%', sub: 'Per month', icon: CreditCard, color: 'text-brand', bg: 'bg-brand/10 dark:bg-brand/20' },
                    { label: 'Monthly Churn', value: '6%', trend: '-1.2%', sub: 'Patient cancellation', icon: PieChart, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-900/20' },
                    { label: 'Patient LTV', value: '$6,667', trend: '+18%', sub: 'Lifetime Value (est)', icon: Target, color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-900/20' },
                ].map((stat, i) => (
                    <div key={i} className="bg-white dark:bg-slate-800 dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm relative overflow-hidden group">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-black ${stat.trend.startsWith('+') ? 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400' : 'bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400'}`}>
            ...
            <div className="grid grid-cols-12 gap-8">
                {/* MRR Trend */}
                <div className="col-span-12 lg:col-span-6 bg-white dark:bg-slate-800 dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-slate-900 dark:text-white dark:text-slate-100 leading-none uppercase tracking-tight">MRR Trend</h3>
                            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-widest">Revenue growth over 12 months</p>
                        </div>
                    </div>
                ...
                <div className="col-span-12 lg:col-span-6 bg-white dark:bg-slate-800 dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-slate-900 dark:text-white dark:text-slate-100 leading-none uppercase tracking-tight">Acquisition Channels</h3>
                            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-widest">New patients by source</p>
                        </div>
                    </div>
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="grid grid-cols-12 gap-8 pb-12">
                ...
                <div className="col-span-12 lg:col-span-4 bg-white dark:bg-slate-800 dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm flex flex-col">
                    <h3 className="text-lg font-black text-slate-900 dark:text-white dark:text-slate-100 leading-none uppercase tracking-tight mb-8">Revenue by Service</h3>
                            ...
                            {SERVICE_DATA.map((s, i) => (
                                <div key={i} className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300 dark:text-slate-400 uppercase tracking-widest">{s.name}</span>
                                    </div>
                                    <span className="text-xs font-black text-slate-900 dark:text-white dark:text-slate-100">{s.value}%</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="col-span-12 lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                        ...
                        <div key={i} className="bg-slate-50/50 dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 dark:border-slate-800 shadow-sm flex items-center gap-6">
                            <div className="w-14 h-14 bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-2xl flex items-center justify-center shadow-sm">
                                <m.icon className={`w-6 h-6 ${m.color}`} />
                            </div>
                            <div>
                                <div className="text-xl font-black text-slate-900 dark:text-white dark:text-slate-100 tracking-tight">{m.value}</div>
                                <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{m.label}</div>
                                <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 mt-0.5">{m.sub}</p>
                            </div>
                        </div>
                    ))}

                    {/* Retention / Cohort Preview */}
                    <div className="md:col-span-2 bg-indigo-600 dark:bg-indigo-700 p-8 rounded-[2rem] shadow-xl shadow-indigo-200 dark:shadow-none flex items-center justify-between text-white overflow-hidden relative">
