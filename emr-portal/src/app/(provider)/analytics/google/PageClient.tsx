...
export default function GoogleAnalyticsDashboard() {
    ...
    const sourceMap = viewData.reduce((acc, curr: any) => {
    ...
    const timeSeriesData = viewData.map((d: any) => {
    ...
    // Totals
    const totalUsers = viewData.reduce((sum, d: any) => sum + (d.users || 0), 0);
    const totalSessions = viewData.reduce((sum, d: any) => sum + (d.sessions || 0), 0);
    const totalViews = viewData.reduce((sum, d: any) => sum + (d.views || 0), 0);
