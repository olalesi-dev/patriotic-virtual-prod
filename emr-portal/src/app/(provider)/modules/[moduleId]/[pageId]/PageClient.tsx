...
export function ModulePageClient({ moduleId, pageId }: { moduleId: string, pageId: string }) {
    ...
    const specialtyModule = SPECIALTY_MODULES.find(m => m.id === moduleId);
    const page = specialtyModule?.pages.find(p => p.id === pageId);

    if (!specialtyModule || !page) {
        return (
    ...
    const Icon = specialtyModule.icon;
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="relative z-10 flex gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="bg-sky-100 text-sky-600 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-sky-200">{specialtyModule.name} Module</span>
                        </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-12 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm text-center flex flex-col items-center">
                ...
                <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">{specialtyModule.name} Active</h2>
