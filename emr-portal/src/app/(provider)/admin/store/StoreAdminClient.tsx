                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <div>
                        ...
                        <div className="flex gap-4 items-start">
                            <label className="w-16 h-16 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 flex items-center justify-center text-slate-400 hover:text-indigo-500 hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 cursor-pointer transition-all shrink-0">
                                <input 
                                    type="file" 
                                    className="hidden" 
                                    accept="image/*"
                            ...
                            <div className="flex gap-2 flex-wrap flex-1">
                                {formData.images?.map((img, i) => (
                                    <div key={i} className="relative w-16 h-16 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden shrink-0 group">
                                        <img src={img} className="w-full h-full object-cover" alt="" onError={e => (e.currentTarget.style.display='none')} />
                                        <button 
                                            onClick={() => setFormData(prev => ({ ...prev, images: prev.images?.filter((_, idx) => idx !== i) }))}
                                            className="absolute inset-0 bg-red-500/80 text-white items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity flex"
                                        >
