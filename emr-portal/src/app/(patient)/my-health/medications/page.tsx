    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Prescriptions</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredMeds.length > 0 ? filteredMeds.map((med) => (
                    <div key={med.id} className="bg-white dark:bg-slate-800 rounded-[32px] border border-slate-50 shadow-sm hover:shadow-xl hover:shadow-sky-900/5 transition-all group overflow-hidden flex flex-col">
                        <div className="p-8 pb-6 flex-1">
                            ...
                            <h3 className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight mb-1">{med.name}</h3>
                        ...
                        <div className="p-4 bg-slate-50/50 border-t border-slate-50 flex gap-2">
                            <button
                                ...
                                className="flex-1 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 py-3 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-50 transition-all flex items-center justify-center gap-2"
                            >
                    ...
                    <div className="col-span-full py-20 text-center space-y-4 bg-white dark:bg-slate-800 rounded-[40px] border border-slate-50">
                        <div className="w-16 h-16 bg-slate-50 dark:bg-slate-900/50 rounded-full flex items-center justify-center text-slate-200 mx-auto">
                        ...
                        <div className="p-10 space-y-8">
                            <div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                                    <p className="text-slate-700 dark:text-slate-200 font-bold leading-relaxed">{selectedMed.instructions || 'Apply as directed by your physician.'}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-8 border-t border-slate-50 pt-8">
                                <div>
                                    ...
                                    <p className="font-black text-slate-800 dark:text-slate-100">{selectedMed.prescribingProvider}</p>
                                </div>
                                <div>
                                ...
                                {aiInfo && (
                                    <div className="mt-6 bg-[#F5F3FF] p-8 rounded-[40px] border border-purple-100 animate-in fade-in slide-in-from-top-4 duration-500 relative overflow-hidden">
                                        <div className="flex items-start gap-4 mb-6">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-black text-slate-800 dark:text-slate-100 tracking-tight">Clinical Assistant</h4>
                                        ...
                                        <div className="space-y-6">
                                            <div>
                                                ...
                                                <p className="text-slate-700 dark:text-slate-200 font-bold leading-relaxed">{aiInfo.action}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest mb-2">Safety & Precautions</p>
                                                <p className="text-slate-700 dark:text-slate-200 font-bold leading-relaxed">{aiInfo.warnings}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest mb-2">Patient Success Tips</p>
                                                <p className="text-slate-700 dark:text-slate-200 font-bold leading-relaxed italic">{aiInfo.tips}</p>
                                            </div>
                                        </div>
